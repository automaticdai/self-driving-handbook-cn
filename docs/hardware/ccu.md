...
!!! info "本节包含"
    - [芯片与平台选型](ccu_soc_selection.md) — 主流 SoC 对比、算力评估方法、工具链与供应链风险、选型决策矩阵
    - [异构计算与车规要求](ccu_arch_safety.md) — 内存层次与数据搬运优化、ISO 26262、硬件安全机制、故障注入测试
    - [自动驾驶计算芯片](compute_chips.md) — 各厂商芯片规格、算力需求分析与发展趋势（芯片参数以该页为准）
...
- **制程**：4nm（台积电 N4）
- **AI 算力**：1000 TOPS（INT8）/ 2000 TFLOPS（FP4）
- **GPU**：Blackwell 架构，原生支持 FP8/FP4 推理
- **目标场景**：L2++/L3/L4，中央计算架构（将座舱与驾驶计算合并）
- **接口**：支持以太网 TSN、PCIe Gen5、DisplayPort 2.1
- **量产状态**：已量产，全新一代奔驰 CLA 为首个搭载完整 NVIDIA DRIVE AV 软件栈的车型（2026 年交付）；双 Thor 组成的 DRIVE AGX Hyperion 10 面向 L4 与 Robotaxi
- **特点**：相比 Orin 的 INT8 算力提升约 4 倍，支持端到端大模型与 VLA 在车端实时推理。
...
- **特点**：算力大幅提升，为 FSD V12–V14 端到端神经网络架构（视频级输入直接输出控制量）提供支撑；2026 年投产的 Cybercab 亦沿用该平台。

!!! note "AI4 与 AI5 不是同一代"
    HW4 对应的自研芯片代号是 **AI4**；**AI5（HW5）** 是 2026 年 4 月才流片的下一代产品，两者不可混用。AI5 的规格与量产节奏见 [自动驾驶计算芯片](compute_chips.md)。
...
- **型号矩阵**：J6B（10+ TOPS）、J6L（~30-40 TOPS）、J6E（80 TOPS）、J6M（128 TOPS）、J6H（约 256 TOPS）、J6P（560 TOPS）
- **发布时间**：6E/6M 于 2024 年量产；6H/6P 于 2025 年 4 月发布，6P 于 2025 年 Q3 首发奇瑞星纪元
- **架构升级**：BPU 第四代"纳什"，支持 Transformer 类模型加速，显著提升端到端模型推理效率
- **目标场景**：覆盖入门 ADAS 至全场景城区辅助驾驶，J6P 面向旗舰高阶智驾并配套 HSD 全栈方案
...
|---------|------|------|---------|------|----------|----------------|------------|
| Orin X  | NVIDIA | 三星 8nm | 254 TOPS | 45 W | 204 GB/s | 理想、蔚来、小鹏 | ~$200（芯片裸价） |
| Thor    | NVIDIA | 4nm  | 1000 TOPS（INT8） | ~100-150 W | 未公开 | 奔驰 CLA 等 | 未公开 |
| FSD（HW3）| Tesla | 14nm | 144 TOPS | 72 W | 68 GB/s | Tesla 自用 | 内部成本 |
| AI4（HW4）| Tesla | 7nm 以下 | 300+ TOPS | ~80 W | 未公开 | Tesla 自用 | 内部成本 |
| EyeQ6H | Mobileye | 7nm | 34 TOPS | < 15 W | 未公开 | 宝马、通用 | < $100 |
| 征程 5  | 地平线 | 16nm | 128 TOPS | 30 W | 51.2 GB/s | 理想（低配）、长城 | ~￥300 |
| 征程 6P | 地平线 | 未公开 | 560 TOPS | ~70 W | 未公开 | 奇瑞星纪元等 | 未公开 |
