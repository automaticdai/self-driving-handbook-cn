...
!!! note "世界模型（World Model）与仿真的融合"
    以 Wayve 的 **GAIA 系列**（GAIA-1 → GAIA-3）、NVIDIA 的 **Cosmos** 与 Waymo 基于 Genie 3 的 **Waymo World Model** 为代表的 **世界模型** 技术，正在模糊传统仿真与神经网络生成之间的界限。世界模型通过学习驾驶场景的时空分布，能够生成逼真的未来驾驶场景，为仿真提供了一种全新的范式。2025–2026 年这条路线的重心已从"生成训练数据"转向"提供可重复的评测环境"——详见 [世界模型](../algorithm/world_models.md)。
