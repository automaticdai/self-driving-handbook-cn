...
在讨论量产架构之前，先看一个改装研究平台的真实配置——它把上述抽象需求落到了具体型号与接线上：

![研究用自动驾驶平台的传感器布置](assets/research-av-sensor-layout.png)

![研究用自动驾驶平台的硬件拓扑与供电/通信连接](assets/research-av-hardware-topology.png)

两图值得注意的工程细节：

- **时间同步靠 GNSS 授时**：GPS 接收机把 GPRMC 报文与 PPS 秒脉冲送给 LiDAR，使点云时间戳与全局时间对齐——这是多传感器融合的前提（详见 [摄像头同步与可靠性](camera_sync_reliability.md)）
- **供电与数据链路分离**：图中橙色为供电、黑色为数据，改装平台的电源设计往往比数据链路更容易出问题
- **工控机而非车规 SoC**：研究平台普遍使用带独立显卡的工控机（此处为 Neousys + GTX1080），代价是功耗、体积与车规可靠性全部不达标——这正是量产必须转向车规 SoC 的原因
...
| Orin X | NVIDIA | 三星8nm | 254 TOPS | 65W | 3.9 | 量产 |
| Thor | NVIDIA | 4nm | 1000 TOPS（INT8） | ~150W | ~6.7 | 已量产（奔驰CLA首发） |
| 征程6P | 地平线 | 未公开 | 560 TOPS | ~70W | ~8.0 | 量产（2025 Q3 SOP） |
...
| 地平线征程6P | ~70W | 560 TOPS | ~8.0 |
| NVIDIA Thor | ~150W | 1000 TOPS（INT8） | ~6.7 |

地平线征程6系列凭借自研BPU（Brain Processing Unit）架构，在TOPS/W效率方面处于行业领先水平。

!!! warning "口径一致性"
    上表统一采用 **INT8** 峰值算力。Thor 常被引用的"2000"实为 FP4 精度下的 TFLOPS，不能与 Orin/征程的 INT8 TOPS 直接相比。芯片选型的完整讨论见 [自动驾驶计算芯片](compute_chips.md)。
...
小鹏最初宣布采用NVIDIA Thor，但于2025年初转向全面自研——此后所有全新车型均搭载自研的**图灵（Turing）AI芯片**，不再规划Thor方案。这是"车企自研芯片"路线中最彻底的一次切换。

- **处理器：** 小鹏图灵芯片，单颗约750 TOPS；G7 Ultra采用三颗方案（综合约2 200 TOPS），2025年6月交付
- **决策依据：** 单颗图灵算力显著高于Orin-X（254 TOPS），略低于Thor中配版，但性价比更优，且可同时用于汽车、飞行汽车与机器人等自有AI硬件
- **传感器配置：** 支持纯视觉方案（无激光雷达）的XNGP城市辅助驾驶
- **通信架构：** 基于X-EEA 3.0集中式电子电气架构，配合中央超算与区域控制器

!!! note "自研 vs 外购的分水岭"
    小鹏的切换说明大算力芯片的竞争已不只是TOPS：Thor的量产延期给了自研方案窗口期，而车企自研可以按自家算法的算子分布裁剪面积、省掉为通用能力付费的部分。各家路线对比见 [中国本土玩家](../casestudy/chinese_players.md)。
