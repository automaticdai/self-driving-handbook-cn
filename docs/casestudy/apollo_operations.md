...
| 规模扩展 | 2024–2025 | 武汉 3000+ 辆投入，实现全市开放区域覆盖 |
| 全国铺开与出海 | 2025–2026 | 覆盖 27 个城市；进入阿布扎比、迪拜、瑞士、韩国、中国香港 |

### 1.2 萝卜快跑运营数据

```
全局规模（2026 年第一季度）：
  · 覆盖城市：27 个（中国 + 中东/欧洲/东亚）
  · 累计公众订单：>2,200 万次
  · 单季全无人订单：320 万次
  · 周订单峰值：>35 万单（2026 年 3 月）
  · 累计自动驾驶里程：>3.3 亿公里

武汉运营情况（2024 年中，作为单城市样本）：
  · 投放车辆：~3000 辆
  · 日均订单：>10,000 单
  · 运营时间：06:00–23:00
...
!!! warning "云端链路是全车队级单点失效"
    2026 年 3 月 31 日晚，武汉约上百辆萝卜快跑因疑似云端通信故障**同时停在主干道路中**，造成数小时拥堵（无人员伤亡）。

    这暴露了一个容易被忽视的架构风险：远程协助与云端调度提升了运营效率，但也让**云侧故障成为全车队级别的单点失效**。当连接中断时，若车辆的默认行为是"原地等待远程指令"，则故障会立即转化为大规模道路阻塞。

    工程启示：车端必须具备**通信降级下的自治能力**——在失去云端连接后，车辆应能自主判断并执行最小风险策略（如驶离主车道后靠边停车），而不是停在原地等待。这与传统功能安全中的"失效可运行（fail-operational）"要求一致，只是失效对象从传感器/计算单元扩展到了整条云端链路。
