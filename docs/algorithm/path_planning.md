...
!!! info "本节包含"
    - [全局路由与行为规划](path_planning_global_behavior.md) — 路网图模型、路由搜索算法、行为规划、路口与并线策略、失效兜底
    - [局部轨迹与速度规划](path_planning_local_trajectory.md) — Frenet 坐标系、轨迹生成、S-T 图速度规划、约束体系、求解器选型与实时性
...
下表汇总了历史上主要路径规划算法的提出背景与优缺点，可作为选型时的快速索引——本页后续章节会展开其中在自动驾驶中仍在实际使用的几类。

![主要路径规划算法的原理与优缺点对比](assets/path-planning-algorithm-comparison.png)
